...
/** Useful for storing a std::pmr::set of longs, serializing to/from
...
{
    std::pmr::set<std::int64_t> m_setData;
...
public:
    explicit NumList(const std::pmr::set<std::int64_t>& theNumbers);
    explicit NumList(std::pmr::set<std::int64_t>&& theNumbers);
    ...
    /** if false, means the numbers were already there. (At least one of them.)
     */
    auto Add(const std::pmr::set<std::int64_t>& theNumbers) -> bool;

    /** if false, means the numbers were NOT already there. (At least one of
     * them.) */
    auto Remove(const std::pmr::set<std::int64_t>& theNumbers) -> bool;

    /** True/False, based on whether values are already there. (ALL theNumbers
     * must be present.) */
    auto Verify(const std::pmr::set<std::int64_t>& theNumbers) const -> bool;
    ...
    auto VerifyAny(const std::pmr::set<std::int64_t>& setData) const -> bool;
    ...
    auto Output(std::pmr::set<std::int64_t>& theOutput) const -> bool;
