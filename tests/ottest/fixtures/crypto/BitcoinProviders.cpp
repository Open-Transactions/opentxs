...
auto Bitcoin_Providers::test_base58_encode() -> bool
{
    auto out = true;
...
auto Bitcoin_Providers::test_base58_decode() -> bool
{
    auto out = true;
...
auto Bitcoin_Providers::test_ripemd160() -> bool
{
    auto out = true;
...
auto Bitcoin_Providers::get_seed(const ot::UnallocatedCString& hex) const
    -> ot::Secret
{
...
auto Bitcoin_Providers::test_bip32_seed(const ot::crypto::Bip32& library)
    -> bool
{
...
auto Bitcoin_Providers::compare_private(
...
auto Bitcoin_Providers::compare_public(
    const ot::crypto::Bip32& library,
    const std::string_view lhs,
    const std::string_view rhs) const -> bool
{
...
auto Bitcoin_Providers::test_bip32_child_key(const ot::crypto::Bip32& library)
    -> bool
{
...
auto Bitcoin_Providers::test_bip39(const ot::crypto::Bip32& library) -> bool
{
    for (const auto& [hex, value] : bip_39_) {
        ...
        const auto entropy = client_.Factory().SecretFromBytes(data.Bytes());
        ...
        const auto targetRoot = client_.Factory().SecretFromBytes(data.Bytes());
