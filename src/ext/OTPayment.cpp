...
OTPayment::OTPayment(const api::Core& core)
    ...
    , m_SenderNymID(identifier::Nym::Factory())
    , m_SenderAcctID(Identifier::Factory())
    , m_RecipientNymID(identifier::Nym::Factory())
    , m_RecipientAcctID(Identifier::Factory())
    , m_RemitterNymID(identifier::Nym::Factory())
    ...
    const identifier::Nym& theNymID) const
{
    ...
    switch (m_Type) {
        ...
        case OTPayment::CHEQUE:
        case OTPayment::INVOICE:
            if (theNymID.operator==(m_SenderNymID)) {
                lOutput =
        ...
        case OTPayment::VOUCHER:
            if (theNymID.operator==(m_RemitterNymID)) {
                lOutput =
                    m_lTransactionNum;  // The "opening" number for a cheque is
                                        // the ONLY number it has.
                bSuccess = true;
...
bool OTPayment::IsCancelledCheque()
{
    ...
    auto sender = identifier::Nym::Factory();
    auto recipient = identifier::Nym::Factory();
    Amount amount{0};
