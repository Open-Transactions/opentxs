...
RSA* RSA::clone() const
{
    ...
    if (!m_p_ascKey->Exists()) {
        if (!key->m_p_ascKey->Exists()) { key->m_p_ascKey->empty(); }
...
// virtual
bool RSA::IsEmpty() const { return (m_p_ascKey->Get()); }
        ...
        if (nullptr != pClearKey) {
            ...
            if (0 == nWriteBio)
                ...
                if (nSize > 0) {
                    ...
                    //
                    Armored::Factory(m_p_ascKey)->SetData(theNewData);  // <======== Success!
                    bReturnVal = true;
                } else
                    otErr << __FUNCTION__
...
RSA::~RSA()
{
    ...
    // form.)
    //
    if (m_p_ascKey->Exists()) m_p_ascKey->empty();
    m_p_ascKey = Armored::Factory();
