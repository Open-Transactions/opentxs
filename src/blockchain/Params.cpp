...
namespace opentxs::blockchain::params
{
ChainData::Checkpoint::Checkpoint(const Data& data) noexcept
    : height_(data.checkpoint_.height_)
    , block_(IsHex, data.checkpoint_.block_hash_)
    , previous_block_(IsHex, data.checkpoint_.previous_block_hash_)
    , cfheader_(IsHex, data.checkpoint_.filter_header_)
{
}
...
public:
    ...
    const ChainData::Checkpoint checkpoint_;
    ...
    ChainDataPrivate(const Data& data, blockchain::Type chain) noexcept
        : chain_(chain)
        , data_(data)
        ...
        , genesis_hash_(IsHex, data.genesis_hash_hex_)
        , checkpoint_(data_)
        ...
        , styles_(data.styles_)
        , genesis_block_()
    {
    }
    ChainDataPrivate(blockchain::Type chain) noexcept
        : ChainDataPrivate(Chains().at(chain), chain)
    {
    }
...
auto ChainData::Checkpoints() const noexcept -> const Checkpoint&
{
...
{
    for (const auto& [style, allowed] : imp_->scripts_) {
