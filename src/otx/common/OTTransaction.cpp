...
auto OTTransaction::VerifyAccount(const identity::Nym& theNym) -> bool
{
    ...
    // todo security audit:
        ...
        LogError()()("Error verifying signature on parent ledger "
    ...
    const PasswordPrompt& reason) -> bool
{
    ...
    if (!OTDB::Exists(
            ...
            filename,
            "")) {
        LogDetail()()("Receipt file doesn't exist in ")(filename).Flush();
    ...
    if (strFileContents.length() < 2) {
        LogError()()("Error reading "
                     "transaction statement: ")(szFolder1name)('/')(
    ...
    if (false == bool(account)) {
        LogConsole()()("Failed loading or verifying account for THE_NYM.")
            .Flush();
    ...
    if ((!pInbox) || (!pOutbox)) {
        LogConsole()()("Inbox or outbox was nullptr after THE_ACCOUNT.Load.")
            .Flush();
    ...
    if (false == bool(pResponseBalanceItem)) {
        ...
        LogConsole()()("Error: atBalanceStatement found on receipt, but not a "
                       "successful one.")
            .Flush();

        return false;
    } else if (!pResponseBalanceItem->VerifySignature(SERVER_NYM)) {
        LogConsole()()("Unable to verify signature on atBalanceStatement item.")
            .Flush();
    ...
    if (false == bool(pBalanceItem)) {
        ...
        LogConsole()()("Unable to verify signature on balanceStatement item.")
            .Flush();
    ...
    if (!serialized->Exists()) {
        LogConsole()()("Unable to load message nym.").Flush();
    ...
    LogDetail()()("Number of inbox/outbox items on the balance statement: ")(
    ...
    for (std::int32_t i = 0; i < pBalanceItem->GetItemCount(); i++) {
        ...
        switch (pSubItem->GetType()) {
            ...
            default: {
                ...
                LogDebug()()("Ignoring ")(strItemType.get())(
        ...
        if (false == bool(pTransaction)) {
            ...
            LogConsole()()("Expected ")(pszLedgerType)(" transaction (")(
    ...
    if (nOutboxItemCount != pOutbox->GetTransactionCount()) {
        LogConsole()()("Outbox mismatch in expected transaction count."
    ...
    for (std::int32_t i = 0; i < pInbox->GetTransactionCount(); i++) {
        ...
        if (missing) {
            LogError()()("Error verifying if transaction num in inbox (")(
                pTransaction->GetTransactionNum())(
    ...
    if (lActualDifference != lInboxSupposedDifference) {
        const auto unittype =
            api_.Storage().Internal().AccountUnit(GetRealAccountID());
        LogError()()("lActualDifference (")(lActualDifference, unittype)(
    ...
    if (wrongBalance) {
        const auto unittype =
            api_.Storage().Internal().AccountUnit(GetRealAccountID());
        LogError()()("lActualDifference in receipts (")(
...
auto OTTransaction::DeleteBoxReceipt(Ledger& theLedger) -> bool
{
    ...
    if (raw_file_->Exists()) {
        ...
        if (false ==
            ...
            LogError()()("Error deleting (writing over) box receipt (failed "
    ...
    bool bDeleted = OTDB::StorePlainString(
        ...
        strFilename->Get());
    if (!bDeleted) {
        LogError()()("Error deleting (writing over) file: ")(
...
auto OTTransaction::SaveBoxReceipt(std::int64_t lLedgerType) -> bool
{
    ...
    if (IsAbbreviated()) {
        LogConsole()()("Unable to save box receipt ")(GetTransactionNum())(
    ...
    if (!bSaved) {
        LogError()()("Error writing file: ")(strFolder1name.get())('/')(
            strFolder2name.get())('/')(strFolder3name.get())('/')(
            strFilename.get())(". Contents: ")(raw_file_.get())(".")
            .Flush();
    }
...
auto OTTransaction::VerifyBoxReceipt(OTTransaction& theFullVersion) -> bool
{
    ...
    if (GetTransactionNum() != theFullVersion.GetTransactionNum()) {
        LogError()()("Failure: The purported 'full version' of the transaction "
    ...
    const PasswordPrompt& reason) -> bool
{
    ...
    if (NYM_ID != GetNymID()) {
        LogError()()("Wrong owner passed.").Flush();
        return false;
    }
    ...
    -> std::shared_ptr<Item>
{
    if (GetItemCountInRefTo(lReference) > 1) {
        LogAbort()()(
            "CAN'T USE GetItemInRefTo! (There are multiple items in reference "
            "to the same number...) SWITCH to using NumberOfOrigin?")
            .Abort();
    }
    ...
    *pbIsSuccess = false;
    ...
    for (auto& it : list_items_) {
        ...
        switch (pItem->GetType()) {
            ...
            // -----------------------------------------------
            default:
                LogError()()("Wrong transaction type passed.").Flush();
                return false;
        }
    }
...
auto OTTransaction::ProcessXMLNode(irr::io::IrrXMLReader*& xml) -> std::int32_t
{
    ...
    if (strNodeName->Compare("nymboxRecord") ||
        ...
        if (strHash->Exists()) {
            ...
            LogError()()("Missing receiptHash on abbreviated record.").Flush();
        }
                                                   ...
                                                   // fixed here.)
    {
        ...
        if (!strAcctID->Exists() || !strNotaryID->Exists() ||
            !strNymID->Exists()) {
            LogConsole()()("Failure: missing strAcctID (")(strAcctID.get())(
                ") or strNotaryID (")(strNotaryID.get())(") or strNymID (")(
                strNymID.get())(").")
                .Flush();
            return (-1);
        }
        ...
        LogTrace()()("Loaded transaction ")(GetTransactionNum())(
        ...
        if (!LoadEncodedTextField(api_.Crypto(), xml, strData) ||
            !strData->Exists()) {
            LogError()()("Error: transaction item "
                         "field without value.")
                .Flush();
            return (-1);  // error condition
        } else {
            ...
            if (!pItem->LoadContractFromString(strData)) {
                ...
                LogError()()("ERROR: Failed verifying transaction Item: ")(
                    strData.get())(".")
                    .Flush();
                return (-1);
            } else {
    ...
    const PasswordPrompt& reason)
{
    ...
    switch (type_) {
        ...
        default:  // All other types are irrelevant for payment inbox reports
            LogAbort()()("Unexpected ")(GetTypeString())(
                " transaction in payment inbox while making abbreviated "
    ...
    const PasswordPrompt& reason)
{
    ...
    switch (type_) {
        ...
        {
            LogAbort()()("Unexpected ")(GetTypeString())(
                " transaction in expired box while making abbreviated "
                "expired-box record.")
                .Abort();
        }
    }
    ...
    const PasswordPrompt& reason)
{
    ...
    switch (type_) {
        ...
        default:  // All other types are irrelevant for inbox reports
        {
            LogError()()("Unexpected ")(GetTypeString())(
                " transaction "
    ...
    const PasswordPrompt& reason)
{
    ...
    switch (type_) {
        ...
        default: {  // All other types are irrelevant for outbox reports.
            LogAbort()()("Unexpected ")(GetTypeString())(
                " transaction in outbox while making abbreviated outbox "
                "record.")
                .Abort();
        }
    }
    ...
    const PasswordPrompt& reason)
{
    ...
    switch (type_) {
        ...
        {
            LogAbort()()("Unexpected ")(GetTypeString())(
                " transaction in inbox while making abbreviated inbox record.")
                .Abort();
        }
    }  // why not transfer receipt? Because the amount was already removed from
       // your account when you transferred it,
    ...
    const PasswordPrompt& reason)
{
    ...
    switch (type_) {  // These are the types that have an amount (somehow)
        ...
        default:  // All other types are irrelevant for inbox reports
        {
            LogDebug()()("Ignoring ")(GetTypeString())(
...
auto OTTransaction::GetReceiptAmount(const PasswordPrompt& reason) -> Amount
{
    ...
    if (false == bool(pOriginalItem)) {
        LogError()()("Unable to find original item. Should never happen.")
            .Flush();
    ...
    switch (GetType()) {
        case transactionType::notice:  // for paymentPlans AND smartcontracts.
        {
            ...
            if (false != bool(pItem)) {
                ...
                LogError()()("Failed trying to get notice item from notice "
                             "transaction. "
                                               ...
                                               // leaves a paymentReceipt...)
        {
            ...
            if (false != bool(pItem)) {
                ...
                LogError()()("Failed trying to get paymentReceipt item from "
                                                  ...
                                                  // cron item...
            {
                ...
                LogError()()("Unable to load Cron Item. Should never happen. "
    ...
    switch (GetType()) {  // These are the types that have an amount (somehow)
                                               ...
                                               // item, attached.)
        {
            ...
            if (!bLoadContractFromString) {
                ...
                LogError()()("ERROR loading cheque or voucher from string: ")(
                    strCheque.get())(".")
                    .Flush();
            } else {
    ...
    -> bool
{
    ...
    if (false == bool(pOriginalItem)) {
        LogError()()("Couldn't load original item, should never happen.")
            .Flush();
        return false;  // Should never happen, since we always expect one based
                       // on the transaction type.
    }
